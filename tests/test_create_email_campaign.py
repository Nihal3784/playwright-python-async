...
from pages.email_campaign_page import EmailCampaignPage
from utils.helpers import generate_campaign_name
from utils.logger import get_logger


@pytest.mark.asyncio
@allure.feature("Create Email Campaign")
@allure.story("Execute email campaign based on segment audience and verify it is execute properly or not")
async def test_create_email_campaign(page):
    logger = get_logger()
    campaign_name = generate_campaign_name("email")

    logger.info(f"Creating campaign: {campaign_name}")

    login = LoginPage(page)
    dashboard = DashboardPage(page)
    campaign = EmailCampaignPage(page)

    await login.login()

    await dashboard.open_email_campaign()
